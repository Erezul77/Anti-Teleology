...
export class SpinoErrorHandler {
  ...
  private determineRecoveryStrategy(errorType: ErrorType, context: ErrorContext): string {
    const strategies = this.recoveryStrategies.get(errorType)
    return strategies ? strategies.name : 'error_recovery'
  }
  ...
  private matchesEdgeCase(userInput: string, patterns: string[]): boolean {
    ...
    // For empty input detection, check if the input is actually empty or just whitespace
    if (patterns.some(p => p === '' || p === ' ' || p === '   ')) {
      return userInput.trim() === '' || userInput.trim().length === 0
    }
    
    // For other patterns, use includes check
  ...
  private initializeEdgeCaseHandlers(): void {
    ...
    this.edgeCaseHandlers.set('empty_input', {
      ...
      fallbackResponse: "Please provide some input so I can help you.",
      confidence: 0.9
    })
    ...
    this.edgeCaseHandlers.set('very_long_input', {
      ...
      fallbackResponse: "Your input is quite long. Let's focus on the most important aspect.",
      confidence: 0.8
    })
  }
