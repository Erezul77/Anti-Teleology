...
import type { PatternV1 } from '../engine/patternIO';
...
export type Preset = {
  ...
  generate: (opts: { x:number; y:number; z:number; seed?: number }) => PatternV1;
...
// ---- Generators ----
function genRandomSeedLife({x,y,z, seed=42}: {x:number;y:number;z:number;seed?:number}): PatternV1 {
  const N = x*y*z;
  const channel = new Uint8Array(N);
  const r = rnd(seed);
  const p = 0.15; // 15% filled
  for (let i=0;i<N;i++) channel[i] = r() < p ? 255 : 0;
  return { schema: 'mpl.pattern.v1', size: {x,y,z}, channel };
}

function genDiffusionInkDrop({x,y,z}: {x:number;y:number;z:number}): PatternV1 {
  const N = x*y*z;
  const channel = new Uint8Array(N);
  const cx = Math.floor(x/2), cy = Math.floor(y/2), cz = Math.floor(z/2);
  const radius = Math.floor(Math.min(x,y,z) * 0.18);
  const r2 = radius*radius;
  for (let zz=0; zz<z; zz++) for (let yy=0; yy<y; yy++) for (let xx=0; xx<x; xx++) {
...
function genCrystalSeeds({x,y,z, seed=7}: {x:number;y:number;z:number;seed?:number}): PatternV1 {
  const N = x*y*z;
  const channel = new Uint8Array(N);
  const r = rnd(seed);
...
function genPerlinishNoise({x,y,z, seed=99}: {x:number;y:number;z:number;seed?:number}): PatternV1 {
  ...
  const scale = 0.12;
  for (let zi=0; zi<z; zi++) for (let yi=0; yi<y; yi++) for (let xi=0; xi<x; xi++) {
    const v =
